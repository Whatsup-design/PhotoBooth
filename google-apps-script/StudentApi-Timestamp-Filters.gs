// Deploy this file as the Student API Web App. Required headers:
// รหัส / ID | ชื่อเล่น | Nickname | Paid | Come | format | updatedAt
...
function doGet(e) {
  try {
    ...
    if (action === 'debugHeaders') return debugHeaders()
...
function doPost(e) {
  try {
    ...
    if (action === 'updatePaid') {
      const id = String(body.id || '').trim()
      if (!id) throw new Error('Missing id')
      if (!normalizeBoolean(body.paid)) throw new Error('Paid must be true')

      const result = withSheetLock(() => updatePaidById(id))
    ...
    if (action === 'updateFormat') {
      ...
      // Frame edits deliberately preserve the original Came timestamp.
...
// Run once from the Apps Script editor only if the header is missing.
function ensureFormatColumn() {
  return ensureColumn(HEADERS.FORMAT, 'Format column')
}

// Run once from the Apps Script editor only if the header is missing.
function ensureUpdatedAtColumn() {
  return ensureColumn(HEADERS.UPDATED_AT, 'updatedAt column')
}

function ensureColumn(headerName, label) {
  ...
  const currentIndex = headers.indexOf(headerName)
  if (currentIndex !== -1) {
    return { success: true, message: `${label} already exists`, columnNumber: currentIndex + 1 }
  }

  const columnNumber = lastColumn + 1
  sheet.getRange(1, columnNumber).setValue(headerName)
  return { success: true, message: `${label} added successfully`, columnNumber }
}
...
function createStudent(input) {
  ...
  const updatedAt = input.come ? new Date() : null
  ...
  row[indexes.updatedAt] = updatedAt || ''
...
function markCameWithFormatById(id, format) {
  ...
  // The enclosing script lock makes these fields one atomic staff action.
  ...
  sheet.getRange(found.rowNumber, indexes.updatedAt + 1).setValue(updatedAt)

  return { rowNumber: found.rowNumber, id, come: true, format, updatedAt: updatedAt.toISOString() }
}
...
function markNotCameById(id) {
  ...
  sheet.getRange(found.rowNumber, indexes.updatedAt + 1).clearContent()
  return { rowNumber: found.rowNumber, id, come: false, format: null, updatedAt: null }
}

function updatePaidById(id) {
  const sheet = getSheet()
  const values = sheet.getDataRange().getValues()
  const indexes = getIndexes(cleanHeaders(values[0]))
  const found = findRowById(values, indexes.id, id)
  ...
  // Do not alter updatedAt: payment is not an arrival event.
  sheet.getRange(found.rowNumber, indexes.paid + 1).setValue(true)
  return { rowNumber: found.rowNumber, id, paid: true }
}
...
function updateFormatById(id, format) {
  ...
  // Do not alter updatedAt: frame changes preserve the original Came time.
  sheet.getRange(found.rowNumber, indexes.format + 1).setValue(format)
...
function studentFromRow(row, rowNumber, indexes) {
  ...
  return {
    ...
    updatedAt: toIsoTimestamp(row[indexes.updatedAt]),
  }
}
...
function getIndexes(headers) {
  const indexes = {
    ...
    updatedAt: headers.indexOf(HEADERS.UPDATED_AT),
  }
